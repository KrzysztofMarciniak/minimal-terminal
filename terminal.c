...
const char **get_terminal_buffer(void) {
...
void resize_terminal(int new_rows, int new_cols) {
    ...
    for (int i = 0; i < new_rows; ++i) {
        ...
        if (!new_buf[i]) {
            ...
            for (int j = 0; j < i; ++j) free(new_buf[j]);
            free(new_buf);
            return;
        }
    ...
    for (int i = 0; i < term_rows; ++i) free(buffer[i]);
    free(buffer);
    buffer = new_buf;
    term_rows = new_rows;
    term_cols = new_cols;
    ...
    if (pty_fd != -1) {
        struct winsize ws = { .ws_row = term_rows, .ws_col = term_cols, .ws_xpixel = 0, .ws_ypixel = 0 };
    ...
    for (size_t i = 0; text[i]; ++i) {
        ...
        if (c == '\033') {
            ...
            while (text[i+1] && j < sizeof(seq)-1) {
                seq[j++] = text[++i];
                if ((seq[j-1] >= 'A' && seq[j-1] <= 'Z') || (seq[j-1] >= 'a' && seq[j-1] <= 'z')) {
                    break;
                }
            }
            seq[j] = '\0';
            ...
            if (cursor_row >= term_rows) {
                for (int r = 0; r < term_rows-1; ++r) memcpy(buffer[r], buffer[r+1], term_cols);
