...
void init_rendering() {
    ...
    font = XLoadQueryFont(display, "-misc-fixed-medium-r-normal--13-120-75-75-c-70-iso8859-1");
    ...
    backBuffer = XCreatePixmap(display, window, w, h,
...
void render_screen() {
    ...
    for (int i = 0; i <= rows; i++) {
        ...
        XDrawLine(display, backBuffer, gc,
                  PADDING, y, PADDING + cols*charW, y);
    }
  }
    for (int j = 0; j <= cols; j++) {
        int x = PADDING + j*charW;
        XDrawLine(display, backBuffer, gc,
                  x, PADDING, x, PADDING + rows*charH);
    }
...
void handle_key_event(XKeyEvent *kev) {
    handle_input(kev);
}
...
void process_events() {
    while (XPending(display)) {
        ...
        if (e.type == ClientMessage &&
            (Atom)e.xclient.data.l[0] == wmDelete) {
            render_cleanup();
            exit(0);
        }
        switch (e.type) {
